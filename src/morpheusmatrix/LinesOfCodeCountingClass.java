...
public class LinesOfCodeCountingClass {
	public Repository initRepo(String location) throws Exception {
		...
		// File("/Users/lesyk/Dropbox/runtime-EclipseApplication/test/.git"))
	...
	public void count() throws Exception {
		...
		for (String project : projects) {
			...
			for (String file : files) {
				...
				// for all log.all()
						.addPath(gitLocation).call();
				count = 0;
				for (RevCommit rev : logs) {

					// find the HEAD
					ObjectId lastCommitId = repository.resolve(rev.getId()
							.getName());

					// a RevWalk allows to walk over commits based on some
